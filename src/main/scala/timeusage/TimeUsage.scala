...
object TimeUsage {
  ...
  /** @return SQL query equivalent to the transformation implemented in `timeUsageGrouped`
    ...
    */
  def timeUsageGroupedSqlQuery(viewName: String): String = s"SELECT working, sex, age, ROUND(AVG(primaryNeeds),1) as primaryNeeds, ROUND(AVG(work),1) as work,  ROUND(AVG(other),1) as other FROM $viewName GROUP BY working, sex, age ORDER BY working, sex, age"
